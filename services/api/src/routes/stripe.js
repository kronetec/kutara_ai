...
stripeRouter.get("/demo-status", (req, res) => {
...
stripeRouter.post("/create-checkout-session", async (req, res, n) => {
  try {
    ...
    if (isDemo) {
      ...
      return res.json({
        ok: true, url: `${DOMAIN}?checkout=success&demo=1`, demo: true,
    ...
    if (!customerId) {
      const customer = await stripe.customers.create({
        email: u.email, name: u.name || u.email, metadata: { userId: u.id },
      });
    ...
    res.json({ ok: true, url: session.url, sessionId: session.id, demo: false });
  } catch (e) { n(e); }
});
...
stripeRouter.post("/create-portal-session", async (req, res, n) => {
  try {
    ...
    if (isDemo) {
      ...
      return res.json({
        ok: true, url: `${DOMAIN}?checkout=cancel&demo=1`, demo: true,
        message: "DEMO MODE: Downgraded to Free"
      });
    }
    ...
    const session = await stripe.billingPortal.sessions.create({
      customer: dbUser.rows[0].stripe_customer_id, return_url: DOMAIN,
    });
    res.json({ ok: true, url: session.url, demo: false });
  } catch (e) { n(e); }
});

stripeRouter.post("/webhook", async (req, res) => {
  if (isDemo) return res.json({ received: true, demo: true });
  ...
  try {
    ...
    event = stripe.webhooks.constructEvent(
      typeof raw === "string" ? raw : JSON.stringify(raw), sig, webhookSecret
    );
  } catch (err) {
  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        await pool.query("UPDATE users SET tier='basic', stripe_subscription_id=$1 WHERE id=$2",
      ...
      case "customer.subscription.updated": {
        ...
        if (sub.status === "past_due" || sub.status === "canceled" || sub.status === "incomplete_expired") {
          await pool.query("UPDATE users SET tier='free' WHERE stripe_customer_id=$1 AND tier='basic'", [sub.customer]);
        }
        break;
      }
      case "customer.subscription.deleted": {
        await pool.query("UPDATE users SET tier='free', stripe_subscription_id=NULL WHERE stripe_customer_id=$1",
